...
class Connect4():
    ...
    def __init__(self, width = 7, height = 6, win_con = 4, board = None):
        self.board = board if board is not None else [[0 for _ in range(width)] for _ in range(height)]
        self.RED = Player('Red')
    ...
    def check_win(self, last_position : tuple[int, int], player : Player) -> bool:
        ...
        try: 
            ...
            pass
        try:
            while self[y][x+1+counter_right] is player: # offset by 1, not counting first piece twice
                counter_right += 1
        except IndexError:
            pass
...
class Games(commands.Cog):
    ...
    async def debug(self, ctx, emoji : discord.Emoji):
        ...
        embed = discord.Embed(title=f'A {str(vs)} B', description=(str(emoji) * 7 + '\n') * 6)
